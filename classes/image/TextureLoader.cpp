...
stTexture* MTextureLoader::LoadTexture(const char* inFileName, unsigned int CountX, unsigned int CountY, unsigned int CountToRead, unsigned int& ReturnCount, float inFilterType, float inWrapType, bool UseAtlas)
{
	if(!CountX | !CountY) 
	{
	...
	if(UseAtlas && (CountX > 1 || CountY > 1))
	{
		LogFile<<"Tx loader: Can not use sheets while creatind multiple texture"<<endl;
		return NULL;
	}
	...
	if(CountX == 1 && CountY == 1 && (CountToRead == 0 || CountToRead == 1))
    {
	    ...
	    if(UseAtlas) retTexture[0].SetTiles(ImageWidth, ImageHeight);
